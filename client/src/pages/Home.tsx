...
// Server-authoritative pricing assignment (see AGENTS.md): the browser never chooses
// a variant. The server assigns once; the client caches the assignment id locally and
// re-validates it on every load, so the displayed cohort is always the recorded one.
type ServerPricingAssignment = { id: string; variant: PricingVariant; assigned_price: number };
const PRICING_ASSIGNMENT_KEY = "fi_pricing_assignment";

function readStoredPricingAssignment(): ServerPricingAssignment | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(PRICING_ASSIGNMENT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<ServerPricingAssignment>;
    if (typeof parsed.id === "string" && parsed.id && (parsed.variant === "A" || parsed.variant === "B")) {
      return { id: parsed.id, variant: parsed.variant, assigned_price: typeof parsed.assigned_price === "number" ? parsed.assigned_price : 0 };
    }
    return null;
  } catch {
    return null;
...
async function requestPricingAssignment(assignmentId?: string): Promise<ServerPricingAssignment> {
  const response = await fetch("/api/validation/pricing-assignment", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(assignmentId ? { assignment_id: assignmentId } : {}) });
  if (!response.ok) throw new Error("Pricing assignment unavailable");
  const data = await response.json() as { assignment: ServerPricingAssignment };
...
function usePricingVariant(): { variant: PricingVariant; assignmentId: string | null } {
  const [variant, setVariant] = useState<PricingVariant>("A");
  const [assignmentId, setAssignmentId] = useState<string | null>(null);
  useEffect(() => {
    let active = true;
    const stored = readStoredPricingAssignment();
    requestPricingAssignment(stored?.id)
      .then((assignment) => {
        if (!active) return;
        window.localStorage.setItem(PRICING_ASSIGNMENT_KEY, JSON.stringify(assignment));
        window.localStorage.setItem("fi_pricing_variant", assignment.variant);
        if (!stored || stored.id !== assignment.id) {
          window.localStorage.setItem("fi_pricing_assigned_at", new Date().toISOString());
          track("pricing_variant_assigned", { pricing_variant: assignment.variant, assigned_price: formatNaira(assignment.variant), assignment_id: assignment.id, acquisition_source: getAcquisitionSource() });
        }
        setAssignmentId(assignment.id);
        setVariant(assignment.variant);
      })
      .catch(() => { /* server unreachable: placeholder remains until signup, which is still server-assigned */ });
    return () => { active = false; };
  }, []);
...
export function TrialPage() {
  const { variant, assignmentId } = usePricingVariant();
